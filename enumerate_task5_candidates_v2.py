#!/usr/bin/env python3
"""
Re-enumerate Task 5 candidates with today's patch types included.

The original enumerate_task5_candidates.py had an ALREADY_CORRECTED_TYPES
set that captured pre-May-3 patches but missed the patch types we've
been using through 2026-05-03 and 2026-05-04. This regenerates the
candidates.tsv with the current corpus state.

New types added (recovered from this session's patches):
  ...
  - administrative_reclassification (agency_correspondence reclasses)
  - bundle_split_multi_allottee (bundle records like _003a/_003b)

Also writes the new candidates TSV to a separate path so the original
May 3 manifest is preserved for historical reference.
"""
import json
...
# Comprehensive set of recovery_note types indicating the record has been
# corrected/handled in some way that should remove it from the candidate list.
...
def main():
    ...
    for path in sorted(EXTRACTIONS.glob("*.json")):
        ...
        # Treat the explicit "not in patents database" Shawnee marker as resolved
        allot_resolved_marker = "(not in patents database)" in allot
    ...
    # Console summary
    print("=" * 70)
    print("Task 5 candidate enumeration (post-2026-05-04 patches)")
    print("=" * 70)
    print()
    ...
    print(f"Total candidates remaining:                {len(cat1) + len(cat2) + len(cat3)}")
    print()
    ...
    if cat3:
        print(f"--- CAT_3: Name captured, allotment missing ({len(cat3)}) ---")
        # Group by tribe for easier scanning
        by_tribe = {}
        for e in cat3:
    ...
    # Write TSV manifest (new path so original is preserved)
    manifest_path = OUTPUT_DIR / "candidates_post_20260504.tsv"
    ...
    print(f"Original May 3 manifest preserved at: {OUTPUT_DIR / 'candidates.tsv'}")
