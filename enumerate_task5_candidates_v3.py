#!/usr/bin/env python3
"""
Enumerate Task 5 candidates v3 — fixes filter bugs from v2.

v2 missed several recovery_note types and didn't exclude records we
explicitly decided NOT to patch. This fixes both.

Records that should NOT appear in any CAT bucket:

1. Records with recovery_notes matching any type below (resolved/handled):
   - All original pre-May-3 types
   - All May-3 and May-4 patch types from this session
   - 'task5_cat1_dual_model_with_corpus_cross_reference' (the Alphonse
     Charbonneau / Lucy Sturdevant cross-reference patches)
   - 'administrative_reclassification' (the 6 agency_correspondence
     reclassifications + 2 done earlier)

2. Records explicitly decided to leave as-is:
   - part10_affidavit_004 (Hannah Hardin signature page; excluded from
     CAT_1 batch per 2026-05-03 decision)
   - part3_questionnaire_022 (blank Circular 2464 instruction template;
     no analytical value per 2026-05-04 decision)

Writes manifest to candidates_post_20260504_v3.tsv.
"""
import json
...
# Records explicitly decided to leave unpatched (with reason)
EXPLICITLY_LEFT = {
...
def main():
    ...
    for path in sorted(EXTRACTIONS.glob("*.json")):
        ...
        allot_resolved_marker = "(not in patents database)" in allot or "non-allottee" in allot.lower()
    ...
    print("=" * 70)
    print("Task 5 candidate enumeration v3 (with explicit exclusions)")
    print("=" * 70)
    print()
    ...
    if explicitly_left_found:
        print("Explicitly left (not in any candidate bucket):")
        for fn, reason in explicitly_left_found:
            print(f"  {fn:50s} {reason}")
        print()

    if cat1:
        print(f"--- CAT_1: Both fields missing ({len(cat1)}) ---")
        for e in cat1:
    ...
    manifest_path = OUTPUT_DIR / "candidates_post_20260504_v3.tsv"
